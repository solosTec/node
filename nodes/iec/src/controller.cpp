...
namespace node 
{
		...
		try
		{
			...
			while (!shutdown)
			{
				...
				if (config)
				{
					...
					//
#if BOOST_OS_LINUX
					auto logger = cyng::logging::make_sys_logger("iec", true);
#else
					auto logger = cyng::logging::make_console_logger(mux.get_io_service(), "iec");
#endif
			...
			const auto conf = cyng::vector_factory({
				cyng::tuple_factory(cyng::param_factory("log-dir", tmp.string())
					...
					, cyng::param_factory("client", cyng::tuple_factory(
						cyng::param_factory("default-port", "8000")
						))	//	client
	...
	int controller::run_as_service(controller&& ctrl, std::string const& srv_name)
	{
		...
		case ERROR_SERVICE_ALREADY_RUNNING:
			...
			::OutputDebugString("An instance of the [iec] service is already running.");
			break;
		case ERROR_FAILED_SERVICE_CONTROLLER_CONNECT:
			...
			//
			::OutputDebugString("***Error 1063: The [iec] service process could not connect to the service controller.");
			std::cerr
				<< "***Error 1063: The [iec] service could not connect to the service controller."
				<< std::endl
				;
			break;
		case ERROR_SERVICE_NOT_IN_EXE:
			//	The executable program that this service is configured to run in does not implement the service.
			::OutputDebugString("The [iec] service is configured to run in does not implement the service.");
			break;
		default:
		{
			std::stringstream ss;
