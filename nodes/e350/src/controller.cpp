...
namespace node 
{
			...
			while (!shutdown)
			{
				...
				if (config)
				{
					...
					if (vec.empty()) {
						...
						auto logger = (console)
							? cyng::logging::make_console_logger(mux.get_io_service(), "smf:e350")
							: cyng::logging::make_file_logger(mux.get_io_service(), (log_dir / "smf-e350.log"))
							;
#endif
						...
						CYNG_LOG_INFO(logger, "e350 uptime " << cyng::io::to_str(cyng::make_object(duration)));
	...
	bool start(cyng::async::mux& mux, cyng::logging::log_ptr logger, cyng::object cfg)
	{
		...
		const auto cluster_tag = cyng::value_cast<boost::uuids::uuid>(dom.get("tag"), uuidgen());
