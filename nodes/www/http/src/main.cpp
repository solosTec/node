...
int main(int argc, char **argv) 
{
    ...
    ("config,C", boost::program_options::value<std::string>(&config_file)->default_value(node::get_cfg_name("http")), "specify the configuration file")
    ...
    //	HTTP node options
    //
    boost::program_options::options_description node_options("HTTP");
    ...
    if (vm.count("version"))
    {
		return node::print_version_info(std::cout, "http");
    }
