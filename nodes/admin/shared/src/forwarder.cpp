...
namespace node
{
		...
		, std::string const& country_code)
	{
		...
		{
			//	{("cmd":insert),
			//	("channel":config.meter),
			//	("rec":{
			//	("key":[null]),
			//	("data":{
			//		("maker":SOL),("connection":1),("medium":6),("version":2),("meter":12345678),("tom":2020-12-02T14:57:02.137Z),("source":LEO:8080)

			auto const meter = cyng::value_cast(reader["rec"]["data"].get("meter"), "00000000");
			auto const maker = cyng::value_cast(reader["rec"]["data"].get("maker"), "SOL");
			auto const manufacturer_code = cleanup_manufacturer_code(maker);

			auto const connection = cyng::numeric_cast(reader["rec"]["data"].get("connection"), 1);
			auto const version = cyng::numeric_cast(reader["rec"]["data"].get("version"), 1);
			auto const medium = cyng::numeric_cast(reader["rec"]["data"].get("medium"), 2);	//	electricity

			auto const source = cyng::value_cast(reader["rec"]["data"].get("source"), "");

			auto const mc = sml::gen_metering_code(country_code, meter, tag);

			//
			//	generate a server ID
			//
			auto const server_id = cleanup_server_id(meter
				, manufacturer_code
				, connection == 2	//	true == wired
				, version
				, medium);

			ctx.queue(bus_req_db_insert("TMeter"
				//	generate new key
				, cyng::table::key_generator(tag)
				//	build data vector
				, cyng::table::data_generator(
					...
					"",		//	meter type
					source,		//	parametrierversion (i.e. 16A098828.pse)
					...
					protocol_any)	//	[string] data protocol(IEC, M - Bus, COSEM, ...)
				, 0
				, ctx.tag()));
		}
		else
	...
	std::string cleanup_manufacturer_code(std::string const& manufacturer)
	{
	...
	std::string cleanup_manufacturer_code(cyng::param_map_t const& pm)
	{
		...
		if (pos != pm.end()) {
			...
			return cleanup_manufacturer_code(manufacturer);
		}
		return "???";
	}
