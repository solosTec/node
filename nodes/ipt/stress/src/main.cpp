...
int main(int argc, char **argv) 
{
	...
	generic.add_options()
		...
		("config,C", boost::program_options::value<std::string>(&config_file)->default_value(node::get_cfg_name("stress")), "specify the configuration file")
	...
	//	IP-T node options
	//
	boost::program_options::options_description node_options("stress");
    ...
    try
    {
        ...
        if (vm.count("version"))
        {
            return node::print_version_info(std::cout, "ipt::stress");
        }
