...
namespace node
{
	namespace ipt
	{
			...
			, master_(0)
		{
			...
			//	request handler
			//
			bus_->vm_.async_run(cyng::register_function("network.task.resume", 4, std::bind(&receiver::task_resume, this, std::placeholders::_1)));
			bus_->vm_.async_run(cyng::register_function("bus.reconfigure", 1, std::bind(&receiver::reconfigure, this, std::placeholders::_1)));
