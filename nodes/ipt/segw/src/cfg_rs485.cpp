...
namespace node
{
	...
	boost::asio::serial_port_base::baud_rate cfg_rs485::get_baud_rate() const
	{
		return boost::asio::serial_port_base::baud_rate(cache_.get_cfg<std::uint32_t>(build_cfg_key({ 
			...
			sml::make_obis(sml::OBIS_SERIAL_SPEED, port_idx) }), 2400u));
	}
	...
	{
		return cache_.get_cfg<std::uint64_t>(build_cfg_key({ 
			sml::OBIS_ROOT_SERIAL, 
			sml::make_obis(sml::OBIS_ROOT_SERIAL, port_idx), 
			sml::OBIS_SERIAL_TASK }), 0);
	}

	bool cfg_rs485::set_lmn_task(std::size_t tsk) const
	{
		return cache_.set_cfg<std::uint64_t>(build_cfg_key({ 
			sml::OBIS_ROOT_SERIAL, 
			sml::make_obis(sml::OBIS_ROOT_SERIAL, port_idx), 
			sml::OBIS_SERIAL_TASK }), tsk);
	}
