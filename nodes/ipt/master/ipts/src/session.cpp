...
#include <ipts/src/session.h>
#include <ipts/src/tasks/open_connection.h>
#include <ipts/src/tasks/close_connection.h>
	...
	namespace ipt
	{
		...
		void session::ipt_res_open_connection(cyng::context& ctx)
		{
			...
			else
			{
				...
				CYNG_LOG_ERROR(logger_, vm().tag() << " ipt.res.open.connection - empty sequence/task relation" << +seq);
