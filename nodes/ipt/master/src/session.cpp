...
namespace node 
{
	namespace ipt
	{
		...
		: mux_(mux)
			...
			vm_.register_function("sml.public.open.response", 6, std::bind(&connect_state::sml_public_open_response, &connect_state_, std::placeholders::_1));
		...
		void session::ipt_req_close_connection(cyng::context& ctx)
		{
			...
			if (bus_->is_online())
			{
				ctx.run(cyng::generate_invoke("log.msg.info", "ipt.req.close.connection", frame));
