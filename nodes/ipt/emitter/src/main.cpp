...
int main(int argc, char **argv) 
{
	...
	generic.add_options()
		...
		("config,C", boost::program_options::value<std::string>(&config_file)->default_value(node::get_cfg_name("emitter")), "specify the configuration file")
	...
	//	IP-T node options
	//
	boost::program_options::options_description node_options("emitter");
    ...
    try
    {
        ...
        if (vm.count("version"))
        {
            return node::print_version_info(std::cout, "ipt::emitter");
        }
		...
		if (ifs.is_open())
		{
 		...
 		node::controller ctrl(config_index, pool_size, json_path, "ipt:emitter");
...
// 			return ctrl.show_config();
		...
		{
			::OutputDebugString("start emitter node");
