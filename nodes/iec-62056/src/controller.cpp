...
namespace node 
{
			...
			while (!shutdown)
			{
				...
				if (config)
				{
					...
					else
					{
						...
						//
#if BOOST_OS_LINUX
						auto logger = cyng::logging::make_sys_logger("smf:iec-62056", true);
#else
						...
						auto logger = (console)
							? cyng::logging::make_console_logger(mux.get_io_service(), "smf:iec-62056")
							: cyng::logging::make_file_logger(mux.get_io_service(), (log_dir / "smf:iec-62056.log"))
							;
#ifdef _DEBUG
						if (!console) {
							std::cout << "log file see: " << (log_dir / "smf:iec-62056.log") << std::endl;
						}
#endif
#endif
			...
			const auto conf = cyng::vector_factory({
				cyng::tuple_factory(cyng::param_factory("log-dir", tmp.string())
					...
					, cyng::param_factory("server", cyng::tuple_factory(
						...
						cyng::param_factory("service", "4059")))
	...
	int controller::run_as_service(controller&& ctrl, std::string const& srv_name)
	{
		...
		case ERROR_SERVICE_ALREADY_RUNNING:
			...
			::OutputDebugString("An instance of the [IEC 62056] service is already running.");
			break;
		case ERROR_FAILED_SERVICE_CONTROLLER_CONNECT:
			...
			//
			::OutputDebugString("***Error 1063: The [IEC 62056] service process could not connect to the service controller.");
			std::cerr
				<< "***Error 1063: The [IEC 62056] service could not connect to the service controller."
				<< std::endl
				;
			break;
		case ERROR_SERVICE_NOT_IN_EXE:
			//	The executable program that this service is configured to run in does not implement the service.
			::OutputDebugString("The [IEC 62056] service is configured to run in does not implement the service.");
			break;
		default:
		{
			std::stringstream ss;
