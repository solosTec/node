...
namespace node 
{
			...
			while (!shutdown)
			{
				...
				if (config)
				{
					...
					if (!vec.empty())
					{
						...
						//
#if BOOST_OS_LINUX
						auto logger = cyng::logging::make_sys_logger("smf:mqtt", true);
#else
						...
						auto logger = (console)
							? cyng::logging::make_console_logger(mux.get_io_service(), "smf:mqtt")
							: cyng::logging::make_file_logger(mux.get_io_service(), (log_dir / "smf-mqtt.log"))
							;
#ifdef _DEBUG
						if (!console) {
							std::cout << "log file see: " << (log_dir / "smf:mqtt.log") << std::endl;
						}
#endif
#endif
						...
						CYNG_LOG_INFO(logger, "smf:mqtt uptime " << cyng::io::to_str(cyng::make_object(duration)));
					}
					else
					{
						std::cerr
	...
	int controller::run_as_service(controller&& ctrl, std::string const& srv_name)
	{
		...
		case ERROR_SERVICE_ALREADY_RUNNING:
			...
			::OutputDebugString("An instance of the [mqtt] service is already running.");
			break;
		case ERROR_FAILED_SERVICE_CONTROLLER_CONNECT:
			...
			//
			::OutputDebugString("***Error 1063: The [mqtt] service process could not connect to the service controller.");
			std::cerr
				<< "***Error 1063: The [mqtt] service could not connect to the service controller."
				<< std::endl
				;
			break;
		case ERROR_SERVICE_NOT_IN_EXE:
			//	The executable program that this service is configured to run in does not implement the service.
			::OutputDebugString("The [mqtt] service is configured to run in does not implement the service.");
			break;
		default:
		{
			std::stringstream ss;
