    ...
    namespace http {
        class session : public std::enable_shared_from_this<session> {
                    ...
                    struct work_impl : work {
                        ...
                        void operator()() {
                            boost::beast::http::async_write(
                                self_.stream_,
                                msg_,
                                boost::beast::bind_front_handler(&session::on_write, self_.shared_from_this(), msg_.need_eof()));
                        }
                    };
          ...
          public:
            ...
            using ws_cb =
                std::function<void(boost::asio::ip::tcp::socket &&, boost::beast::http::request<boost::beast::http::string_body>)>;
          ...
          private:
            ...
            void
            handle_get_head_request(boost::beast::http::request<boost::beast::http::string_body> &&, std::string const &, bool);
            ...
            send_bad_request(std::uint32_t version, bool keep_alive, std::string const &why);

            boost::beast::http::response<boost::beast::http::string_body>
            send_not_found(std::uint32_t version, bool keep_alive, std::string target);

            boost::beast::http::response<boost::beast::http::string_body>
            send_server_error(std::uint32_t version, bool keep_alive, boost::system::error_code ec);

            boost::beast::http::response<boost::beast::http::string_body>
            send_redirect(std::uint32_t version, bool, std::string host, std::string target);

            boost::beast::http::response<boost::beast::http::empty_body>
            send_head(std::uint32_t version, bool, std::string const &path, std::uint64_t);

            boost::beast::http::response<boost::beast::http::file_body> send_get(
            ...
            boost::beast::http::response<boost::beast::http::string_body>
            send_204(std::uint32_t version, bool, std::string options);

            boost::beast::http::response<boost::beast::http::string_body>
            send_not_authorized(std::uint32_t version, bool keep_alive, std::string target, std::string realm);

            boost::beast::http::response<boost::beast::http::string_body> send_ip_address(std::uint32_t version, bool keep_alive);
