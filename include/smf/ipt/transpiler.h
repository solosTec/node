...
namespace smf {
    namespace ipt {
        [[nodiscard]] scramble_key to_sk(cyng::buffer_t const &data, std::size_t offset);
        ...
        namespace {
            ...
            //	numerics
            //
            template <typename T> std::size_t read_policy(T &value, cyng::buffer_t const &data, std::size_t offset) {
            ...
            template <std::size_t N, typename... Args> struct read_impl {
            ...
            template <typename... Args> struct read_impl<0, Args...> {
                static std::size_t convert(cyng::buffer_t const &data, std::tuple<Args...> &tpl, std::size_t offset) {
        ...
        template <typename... Args> auto read(cyng::buffer_t &&data) -> std::tuple<bool, std::decay_t<Args>...> {
