...
namespace smf {
	...
	namespace sml {
		 ...
		 */
		constexpr status_word_t to_u64(status_bit sb) {
			return static_cast<status_word_t>(sb);
		}
		...
		constexpr status_word_t get_initial_value() {
			return to_u64(status_bit::_ON)
				| to_u64(status_bit::RESET_BY_WATCHDOG)
				| to_u64(status_bit::EXT_IF_AVAILABLE)
				| to_u64(status_bit::NOT_AUTHORIZED_IPT)
				;
		}

		constexpr bool is_set(status_word_t w, status_bit e)	{
			return (w & to_u64(e)) == to_u64(e);
		}

		constexpr status_word_t set(status_word_t w, status_bit e) 	{
			return w |= to_u64(e);
		}

		constexpr status_word_t remove(status_word_t w, status_bit e)	{
			return w &= ~to_u64(e);
		}
