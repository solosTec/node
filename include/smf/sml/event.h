...
namespace smf {
    namespace sml {
         ...
         * @return event type
         */
        constexpr std::uint32_t evt_get_type(std::uint32_t evt) { return (evt & 0xFF000000) >> 24; }

        /**
         * @return event level
         */
        constexpr std::uint32_t evt_get_level(std::uint32_t evt) { return (evt & 0xF00) >> 8; }

        /**
         * @return event source
         */
        constexpr std::uint32_t evt_get_source(std::uint32_t evt) { return evt & 0xFF; }
        ...
        enum event_sources : std::uint64_t {
            ...
            EVT_SOURCE_07 = 0x8141000000FF, //	IP(Erweiterungs - Schnittstelle): LOG_SOURCE_IP
            EVT_SOURCE_08 = 0x814200000001, //	SML(Erweiterungs--Schnittstelle): LOG_SOURCE_SML_EXT
            EVT_SOURCE_09 = 0x814200000002, //	SML(Kunden - Schnittstelle): LOG_SOURCE_SML_CUSTOM
            EVT_SOURCE_10 = 0x814200000003, //	SML(Service - Schnittstelle): LOG_SOURCE_SML_SERVICE
            EVT_SOURCE_11 = 0x814200000004, //	SML(WAN - Schnittstelle): LOG_SOURCE_SML_WAN
            EVT_SOURCE_12 = 0x814200000005, //	SML(eHZ - Schnittstelle): LOG_SOURCE_SML_eHZ
            EVT_SOURCE_13 = 0x814200000006, //	SML(W - MBUS - Schnittstelle): LOG_SOURCE_SML_wMBUS
            EVT_SOURCE_14 = 0x8145000000FF, //	Push(SML): LOG_SOURCE_PUSH_SML
            EVT_SOURCE_15 = 0x8146000000FF, //	Push(IPT - Quelle): LOG_SOURCE_PUSH_IPT_SOURCE
            ...
            EVT_SOURCE_43 = 0x8146000002ff, //	addressed Profile (PEER_ADDRESS)
            EVT_SOURCE_44 = 0x8181000000FF, //	addressed Profile (PEER_ADDRESS) ?
            //
