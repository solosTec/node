...
namespace smf {
	...
	namespace sml {
		...
		namespace detail
		{
			...
			template<>
			struct factory_policy<std::chrono::minutes>
			{
				static cyng::tuple_t create(std::chrono::minutes v);
			};
			...
			struct factory_policy<char const*>
			{
				static cyng::tuple_t create(char const* p) {
					return factory_policy<std::string>::create(std::string(p));
				}
			};

			template<>
			struct factory_policy<cyng::obis>
			{
				static cyng::tuple_t create(cyng::obis v);
			};
			...
			template <std::size_t N>
			struct factory_policy <cyng::aes_key<N> const&>
			{
