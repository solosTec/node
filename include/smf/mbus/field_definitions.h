...
namespace smf {
    namespace mbus {
        ...
        // constexpr std::uint8_t CTRL_FIELD_IR = 0x00;		//	Send manually initiated installation data – Send
        // installation request constexpr std::uint8_t CTRL_FIELD_ACC_NR = 0x00;	//	No data – Provides opportunity to
        // access the meter between two application transmissions constexpr std::uint8_t CTRL_FIELD_ACC_DMD = 0x00;	//
        // Access demand to master in order to request new importat application data – Alerts constexpr std::uint8_t CTRL_FIELD_ACC
        // = 0x00;		//	Acknowledge the reception constexpr std::uint8_t CTRL_FIELD_RSP_UD = 0x00;	//
        ...
        enum c_field_master : std::uint8_t {
            ...
            CTRL_FIELD_CNF_IR =
                0x06, //	Confirms the successful registration (installation) ofmeter/actuator into this gateway
        };
        ...
        enum ci_field : std::uint8_t {
            ...
            FIELD_CI_LINK_FROM_DEVICE_LONG =
                0x8B, //!<	Link extension from device with long header(OMS Vol.2 Issue 2.0.0/2009-07-20)

            //#if !OMS_V4_ENABLED

            FIELD_CI_EXT_DLL_I =
                0x8C, //!<	Additional Link Layer may be applied for Radio messages with or without Application Layer.
            FIELD_CI_EXT_DLL_II =
                0x8D, //!<	Additional Link Layer may be applied for Radio messages with or without Application Layer (ELL)
        ...
        const char *field_name(ci_field);

        enum class tplayer_type { NONE, SHORT, LONG };

        /**
         * use the CI field to determine the TPL field type
         */
        constexpr tplayer_type get_tpl_type(std::uint8_t ci) noexcept {
            switch (ci) {
            ...
            case FIELD_CI_RES_SHORT_SML: return tplayer_type::SHORT;
            ...
            case FIELD_CI_RES_TLS_LONG: return tplayer_type::LONG;
            case FIELD_CI_HEADER_NO: break;
            default: break;
            }

            return tplayer_type::NONE;
        }
