...
namespace smf {
    ...
    target::target(cyng::channel_weak wp, cyng::controller &ctl, cyng::logger logger, ipt::bus &bus)
        : sigs_{std::bind(&target::register_target, this, std::placeholders::_1), std::bind(&target::receive, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3, std::placeholders::_4), std::bind(&target::add_writer, this, std::placeholders::_1), std::bind(&target::stop, this, std::placeholders::_1)}
        , channel_(wp)
        , ctl_(ctl)
        , logger_(logger)
        , bus_(bus)
    ...
    void target::stop(cyng::eod) {}

    void target::register_target(std::string name) { bus_.register_target(name, channel_); }

    void target::add_writer(std::string name) { writers_.insert(name); }

    void target::receive(std::uint32_t channel, std::uint32_t source, cyng::buffer_t data, std::string target) {
    ...
    void target::open_response(std::string const &trx, cyng::tuple_t const &msg) {
    ...
    void target::close_response(std::string const &trx, cyng::tuple_t const &msg) {
        auto const r = smf::sml::read_public_close_response(msg);
    }
    void target::get_profile_list_response(std::string const &trx, std::uint8_t group_no, cyng::tuple_t const &msg) {
    ...
    void target::get_proc_parameter_response(std::string const &trx, std::uint8_t group_no, cyng::tuple_t const &msg) {
    ...
    void target::get_list_response(std::string const &trx, std::uint8_t group_no, cyng::tuple_t const &msg) {
