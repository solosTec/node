...
namespace smf {
    ...
    void db::set_start_values(cyng::param_map_t const &session_cfg, std::string const &country_code, std::string const &lang_code) {
        ...
        insert_cluster_member(
            cfg_.get_tag(), "main", cyng::version(SMF_VERSION_MAJOR, SMF_VERSION_MINOR), boost::asio::ip::tcp::endpoint(),
            cyng::sys::get_process_id());
    }

    bool db::insert_cluster_member(
        boost::uuids::uuid tag, std::string class_name, cyng::version v, boost::asio::ip::tcp::endpoint ep, cyng::pid pid) {

        return cache_.insert(
            "cluster", cyng::key_generator(tag),
            cyng::data_generator(
                class_name, std::chrono::system_clock::now(), v, static_cast<std::uint64_t>(0), std::chrono::microseconds(0), ep,
                pid),
    ...
    bool db::insert_pty(
        ...
        ,
        std::string const &name, std::string const &pwd, boost::asio::ip::tcp::endpoint ep, std::string const &data_layer) {

        CYNG_LOG_TRACE(
            logger_, "[db] insert session " << name << ", tag: " << tag << ", peer: " << peer << ", remote tag: " << rtag);

        return cache_.insert(
            "session", cyng::key_generator(tag),
            cyng::data_generator(
                peer, name, ep, rtag, source_++ //	source
                ,
                ...
                "ipt" //	player
                ,
                data_layer, static_cast<std::uint64_t>(0), static_cast<std::uint64_t>(0), static_cast<std::uint64_t>(0)),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
    }
    ...
    void db::establish_connection(
        boost::uuids::uuid caller_tag, boost::uuids::uuid caller_vm, boost::uuids::uuid caller_dev, std::string caller_name,
        std::string callee_name, boost::uuids::uuid tag //	callee tag
        ,
    ...
    void db::init_demo_data() {
        ...
        auto b = cache_.insert(
            "device", cyng::key_generator(tag_01),
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_02),
        ...
        b = cache_.insert(
            "meterIEC", cyng::key_generator(tag_01),
            cyng::data_generator(
                "192.168.0.200", static_cast<std::uint16_t>(2000u), std::chrono::seconds(840),
        ...
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_02),
            cyng::data_generator(
                boost::asio::ip::make_address("192.168.0.200"), static_cast<std::uint16_t>(6000u),
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_03),
            cyng::data_generator(
                "MA0000000000000000000000003496219", "secret", "3496219", "Lot Yakut,C1 House 101", "", "", true,
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterIEC", cyng::key_generator(tag_03),
            cyng::data_generator(
                "10.132.28.150", static_cast<std::uint16_t>(6000u), std::chrono::seconds(840),
                std::chrono::system_clock::time_point(std::chrono::hours(0))),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        ...
        //	meter
        b = cache_.insert(
            "meter", cyng::key_generator(tag_03),
            cyng::data_generator(
                ...
                ,
                "NXT4-S20EW-6N00-4000-5020-E50/Q" // cyng::column("item", cyng::TC_STRING),		//	[string] ArtikeltypBezeichnung
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_04),
            cyng::data_generator(
                "MA0000000000000000000000003496225", "secret", "3496225", "C1 House 94", "", "", true,
                std::chrono::system_clock::now()),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterIEC", cyng::key_generator(tag_04),
            cyng::data_generator(
                "10.132.28.151", static_cast<std::uint16_t>(6000u), std::chrono::seconds(840),
                std::chrono::system_clock::time_point(std::chrono::hours(0))),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_05),
            cyng::data_generator(
                "MA0000000000000000000000035074614", "secret", "3496230", "Yasmina,House A2 Meter Board", "", "", true,
                std::chrono::system_clock::now()),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_05),
            cyng::data_generator(
                "35074614" //	ident nummer (i.e. 1EMH0006441734, 01-e61e-13090016-3c-07)
                ,
                "35074614" //	[string] meter number (i.e. 16000913) 4 bytes
                ,
                "MA0000000000000000000000035074614" // cyng::column("code", cyng::TC_STRING),		//	[string] metering code - changed
                                                    // at 2019-01-31
                ,
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterIEC", cyng::key_generator(tag_05),
            cyng::data_generator(
                "10.132.24.150", static_cast<std::uint16_t>(6000u), std::chrono::seconds(840),
                std::chrono::system_clock::time_point(std::chrono::hours(0))),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_06),
            cyng::data_generator(
                "MA0000000000000000000000004354752", "secret", "4354752", "Maison 44", "", "", true,
                std::chrono::system_clock::now()),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_06),
            cyng::data_generator(
                boost::asio::ip::make_address("10.132.32.142"), static_cast<std::uint16_t>(2000u),
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_07),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "gateway", cyng::key_generator(tag_07),
            cyng::data_generator(
                ...
                ,
                "06441734", cyng::mac48() // cyng::column("ifService", cyng::TC_MAC48),		//	(5) MAC of service interface
                ,
        ...
        //	87654329
        b = cache_.insert(
            "device", cyng::key_generator(tag_08),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_08),
            cyng::data_generator(
                boost::asio::ip::make_address("0.0.0.0"), static_cast<std::uint16_t>(12000u),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_08),
            cyng::data_generator(
        ...
        //	16000913
        b = cache_.insert(
            "device", cyng::key_generator(tag_09),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_09),
            cyng::data_generator(
                boost::asio::ip::make_address("0.0.0.0"), static_cast<std::uint16_t>(12000u),
                cyng::make_aes_key<cyng::crypto::aes128_size>(cyng::hex_to_buffer("51728910E66D83F851728910E66D83F8")),
                std::chrono::system_clock::time_point(std::chrono::hours(0))),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_09),
            cyng::data_generator(
                "01-e61e-13090016-3c-07" //	ident nummer (i.e. 1EMH0006441734, 01-e61e-13090016-3c-07)
                ,
                "16000913" //	[string] meter number (i.e. 16000913) 4 bytes
        ...
        //	04453174
        b = cache_.insert(
            "device", cyng::key_generator(tag_10),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_10),
            cyng::data_generator(
                boost::asio::ip::make_address("0.0.0.0"), static_cast<std::uint16_t>(12000u),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_10),
            cyng::data_generator(
        ...
        //	00684279
        b = cache_.insert(
            "device", cyng::key_generator(tag_11),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_11),
            cyng::data_generator(
                boost::asio::ip::make_address("0.0.0.0"), static_cast<std::uint16_t>(12000u),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_11),
            cyng::data_generator(
        ...
        //	21061457
        b = cache_.insert(
            "device", cyng::key_generator(tag_12),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_12),
            cyng::data_generator(
                boost::asio::ip::make_address("0.0.0.0"), static_cast<std::uint16_t>(12000u),
                cyng::make_aes_key<cyng::crypto::aes128_size>(cyng::hex_to_buffer("6140B8C066EDDE3773EDF7F8007A45AB")),
                std::chrono::system_clock::time_point(std::chrono::hours(0))),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_12),
            cyng::data_generator(
        ...
        //	00636408
        b = cache_.insert(
            "device", cyng::key_generator(tag_13),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterwMBus", cyng::key_generator(tag_13),
            cyng::data_generator(
                boost::asio::ip::make_address("0.0.0.0"), static_cast<std::uint16_t>(12000u),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_13),
            cyng::data_generator(
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_14),
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "gateway", cyng::key_generator(tag_14),
            cyng::data_generator(
                ...
                std::chrono::system_clock::now() //	tom
                ,
                "B021774", cyng::mac48() // cyng::column("ifService", cyng::TC_MAC48),		//	(5) MAC of service interface
                ,
                cyng::mac48() // cyng::column("ifData", cyng::TC_STRING),		//	(6) MAC of WAN interface
                ,
                "pw" // cyng::column("pwdDef", cyng::TC_STRING),		//	(7) Default PW
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_15),
            cyng::data_generator(
                "CH0000000000000000000000003218421", "secret", "03218421", "test device", "", "", true,
                std::chrono::system_clock::now()),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        b = cache_.insert(
            "meter", cyng::key_generator(tag_15),
            cyng::data_generator(
                "01-e61e-08646300-36-03" //	ident nummer (i.e. 1EMH0006441734, 01-e61e-13090016-3c-07)
                ,
                "03218421" //	[string] meter number (i.e. 16000913) 4 bytes
                ,
                "CH0000000000000000000000003218421" // cyng::column("code", cyng::TC_STRING),		//	[string] metering code - changed
                                                    // at 2019-01-31
                ,
                "ELS" // cyng::column("maker", cyng::TC_STRING),		//	[string] manufacturer
                ,
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterIEC", cyng::key_generator(tag_15),
            cyng::data_generator(
                "192.168.0.200", static_cast<std::uint16_t>(6006u), std::chrono::seconds(840),
                std::chrono::system_clock::time_point(std::chrono::hours(0))),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_16),
            cyng::data_generator(
                "MA0000000000000000000000035074616", "secret", "3496230", "Yasmina,House A2 Meter Board", "", "", true,
                std::chrono::system_clock::now()),
            1u //	only needed for insert operations
            ,
            cfg_.get_tag());
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meter", cyng::key_generator(tag_16),
            cyng::data_generator(
                "35074616" //	ident nummer (i.e. 1EMH0006441734, 01-e61e-13090016-3c-07)
                ,
                "35074616" //	[string] meter number (i.e. 16000913) 4 bytes
                ,
                "MA0000000000000000000000035074616" // cyng::column("code", cyng::TC_STRING),		//	[string] metering code - changed
                                                    // at 2019-01-31
                ,
                "ELS" // cyng::column("maker", cyng::TC_STRING),		//	[string] manufacturer
                ,
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "meterIEC", cyng::key_generator(tag_16),
            cyng::data_generator(
                "10.132.24.150", static_cast<std::uint16_t>(6000u), std::chrono::seconds(840),
                std::chrono::system_clock::time_point(std::chrono::hours(0))),
            1u //	only needed for insert operations
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_17),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "gateway", cyng::key_generator(tag_17),
            cyng::data_generator(
                ...
                std::chrono::system_clock::now() //	tom
                ,
                "B022980", cyng::mac48() // cyng::column("ifService", cyng::TC_MAC48),		//	(5) MAC of service interface
                ,
                cyng::mac48() // cyng::column("ifData", cyng::TC_STRING),		//	(6) MAC of WAN interface
                ,
                "pw" // cyng::column("pwdDef", cyng::TC_STRING),		//	(7) Default PW
        ...
        b = cache_.insert(
            "device", cyng::key_generator(tag_18),
            cyng::data_generator(
        ...
        BOOST_ASSERT_MSG(b, "insert failed");
        b = cache_.insert(
            "gateway", cyng::key_generator(tag_18),
            cyng::data_generator(
                ...
                std::chrono::system_clock::now() //	tom
                ,
                "B02297e", cyng::mac48() // cyng::column("ifService", cyng::TC_MAC48),		//	(5) MAC of service interface
                ,
                cyng::mac48() // cyng::column("ifData", cyng::TC_STRING),		//	(6) MAC of WAN interface
                ,
                "pw" // cyng::column("pwdDef", cyng::TC_STRING),		//	(7) Default PW
    ...
    std::pair<std::uint32_t, bool> db::register_target(
        boost::uuids::uuid tag, boost::uuids::uuid dev, boost::uuids::uuid peer //	local vm
        ,
        std::string name, std::uint16_t paket_size, std::uint8_t window_size) {
            ...
            [&](cyng::table *tbl_trg, cyng::table const *tbl_dev) {
                ...
                if (!rec.empty()) {
                    ...
                    r = tbl_trg->insert(
                        ...
                        cyng::data_generator(
                            tag, peer, name, dev, owner, paket_size, window_size, std::chrono::system_clock::now(),
    ...
    std::tuple<std::uint32_t, std::uint32_t, std::uint16_t, std::uint32_t> db::open_channel(
        boost::uuids::uuid tag, boost::uuids::uuid dev, std::string name, std::string account, std::string number,
        ...
        cache_.access(
                ...
                if (!rec_session.empty()) {
                    ...
                    for (auto const &key_target : targets) {
                        ...
                        tbl_channel->insert(
                            ...
                            cyng::data_generator(target_tag, target_peer, packet_size, timeout), 1, cfg_.get_tag());
                    }
            ...
            cyng::access::read("session"), cyng::access::read("target"), cyng::access::write("channel"));
    ...
    std::pair<cyng::key_list_t, std::uint16_t> db::get_matching_targets(
        cyng::table const *tbl, std::string name, std::string account, std::string number, std::string sv, std::string id,
    ...
    push_target::push_target() : pty_(boost::uuids::nil_uuid(), boost::uuids::nil_uuid()), channel_(0) {}

    push_target::push_target(pty p, std::uint32_t channel) : pty_(p), channel_(channel) {}

    std::vector<cyng::meta_store> get_store_meta_data() {
        return {
            config::get_config(), //	"config"
            config::get_store_device(),  config::get_store_meter(),    config::get_store_meterIEC(), config::get_store_meterwMBus(),
            config::get_store_gateway(), config::get_store_lora(),     config::get_store_gui_user(), config::get_store_target(),
            config::get_store_cluster(), config::get_store_location(), config::get_store_session(),  config::get_store_connection(),
