    ...
    namespace nms {
        ...
        void server::start(boost::asio::ip::tcp::endpoint ep) {
            ...
            while ((cnt < MAX_COUNT) && (!already_listen)) {
                CYNG_LOG_INFO(logger_, "[NMS] server: loop no.  "
                                           << cnt << " of " << MAX_COUNT - 1);
                cnt++;
                ...
                if (!already_openend) {
                    ...
                    if (ec) {
                        ...
                        CYNG_LOG_INFO(logger_, "[NMS] server: going to sleep  "
                                                   << sleep_interval_seconds
                                                   << " seconds")

                        std::this_thread::sleep_for(
                            std::chrono::seconds(sleep_interval_seconds));
                ...
                if (!already_set_options) {
                    ...
                    if (ec) {
                        ...
                        CYNG_LOG_INFO(logger_,
                                      "[NMS] server: set options sucessful ")
                    }
                }
                if (!already_bind) {
            ...
            if (already_listen) {
                ...
                CYNG_LOG_WARNING(logger_,
                                 "[NMS] server cannot start listening at "
                                     << ep << ": " << ec.message());
            }
        }
        ...
        void server::do_accept() {
            acceptor_.async_accept([this](boost::system::error_code ec,
                ...
                if (!ec) {
                    CYNG_LOG_INFO(logger_, "[NMS] new session "
                                               << socket.remote_endpoint());

                    auto sp = std::shared_ptr<session>(
                        new session(std::move(socket), cfg_, logger_,
                                    std::bind(&server::rebind, this,
                                              std::placeholders::_1)),
                        [this](session *s) {
                            //
                            ...
                            //
                            --session_counter_;
                            CYNG_LOG_TRACE(logger_, "[NMS] session(s) running: "
                                                        << session_counter_);
        ...
        void server::stop() {
            ...
            if (session_counter_ != 0) {
                CYNG_LOG_WARNING(logger_, "[NMS] server "
                                              << session_counter_
        ...
        void server::rebind(boost::asio::ip::tcp::endpoint ep) {
            ...
            acceptor_ =
                boost::asio::ip::tcp::acceptor(acceptor_.get_executor(), ep);
        }
