...
namespace smf {
  namespace nms {
    ...
    server::server(cyng::controller &ctl, cfg &c, cyng::logger logger)
        : cfg_(c), logger_(logger),
          acceptor_(ctl.get_ctx()), session_counter_{0} {}
    ...
    void server::start(boost::asio::ip::tcp::endpoint ep) {
      ...
      while ((cnt < MAX_COUNT) && (!already_listen)) {
        CYNG_LOG_INFO(logger_, "[NMS] server: loop no.  " << cnt << " of "
                                                          << MAX_COUNT - 1);
        cnt++;
        ...
        if (!already_openend) {
          ...
          if (ec) {
            ...
            CYNG_LOG_INFO(logger_, "[NMS] server: going to sleep  "
                                       << sleep_interval_seconds << " seconds")
      ...
      acceptor_.async_accept([this](boost::system::error_code ec,
        ...
        if (!ec) {
          CYNG_LOG_INFO(logger_,
                        "[NMS] new session " << socket.remote_endpoint());

          auto sp = std::shared_ptr<session>(
              new session(
                  std::move(socket), cfg_, logger_,
              ...
              [this](session *s) {
                ...
                --session_counter_;
                CYNG_LOG_TRACE(
                    logger_, "[NMS] session(s) running: " << session_counter_);
