...
void broker::receive(cyng::buffer_t data) {
    if (is_connected()) {
        CYNG_LOG_INFO(logger_, "[broker] transmit " << data.size()
                                                    << " bytes to " << target_);
        ...
        CYNG_LOG_WARNING(logger_, "[broker] drops " << data.size()
                                                    << " bytes to " << target_);
    }
}
...
void broker::handle_connect(
    ...
    if (!socket_.is_open()) {
        ...
        CYNG_LOG_WARNING(logger_,
                         "[broker] " << target_ << " connect timed out");
...
void broker::handle_write(const boost::system::error_code &ec) {
    ...
    if (!ec) {
        ...
        CYNG_LOG_ERROR(logger_, "[broker] " << target_ << " on heartbeat: "
                                            << ec.message());
