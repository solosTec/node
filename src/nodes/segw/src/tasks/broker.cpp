...
namespace smf {
  ...
  broker::broker(cyng::channel_weak wp, cyng::controller &ctl,
                 cyng::logger logger, target const &t, bool login)
      : state_(state::START),
        sigs_{std::bind(&broker::receive, this, std::placeholders::_1),
              std::bind(&broker::check_status, this, std::placeholders::_1),
              std::bind(&broker::stop, this, std::placeholders::_1)},
        channel_(wp), ctl_(ctl), logger_(logger), target_(t), login_(login),
        endpoints_(), socket_(ctl.get_ctx()), dispatcher_(ctl.get_ctx()),
        input_buffer_(), buffer_write_() {
  ...
  void broker::receive(cyng::buffer_t data) {
    if (is_connected()) {
      CYNG_LOG_INFO(logger_, "[broker] transmit " << data.size() << " bytes to "
                                                  << target_);
      ...
      CYNG_LOG_WARNING(logger_, "[broker] drops " << data.size() << " bytes to "
                                                  << target_);
    }
  }
  ...
  void broker::start() {
    ...
    connect(
        r.resolve(target_.get_address(), std::to_string(target_.get_port())));
  }
  ...
  void broker::start_connect(
    ...
    if (endpoint_iter != endpoints_.end()) {
      CYNG_LOG_TRACE(logger_,
                     "[broker] trying " << endpoint_iter->endpoint() << "...");
  ...
  void broker::check_status(std::chrono::seconds timeout) {
    ...
    auto sp = channel_.lock();
    if (sp)
      sp->suspend(timeout, "check-status",
                  cyng::make_tuple(timeout < std::chrono::seconds(10)
  ...
  void broker::handle_connect(
    ...
    // Check if the connect operation failed before the deadline expired.
      ...
      CYNG_LOG_WARNING(logger_, "[broker] " << target_ << " connect error "
                                            << ec.value() << ": "
                                            << ec.message());
    ...
    // Otherwise we have successfully established a connection.
      ...
      CYNG_LOG_INFO(logger_, "[broker] " << target_ << " connected to "
      ...
      if (login_) {
        ...
        boost::asio::post(dispatcher_, [this]() {
          ...
          //	set login sequence
          //
          buffer_write_.emplace_back(
              cyng::make_buffer(target_.get_login_sequence()));
  ...
  void broker::do_read() {
    ...
    boost::asio::async_read_until(
        ...
        std::bind(&broker::handle_read, this, std::placeholders::_1,
                  std::placeholders::_2));
  }
  ...
  void broker::handle_read(const boost::system::error_code &ec, std::size_t n) {
    ...
    if (!ec) {
      ...
      CYNG_LOG_ERROR(logger_, "[broker] " << target_ << " read " << ec.value()
                                          << ": " << ec.message());
      reset();
    }
  }
  ...
  void broker::handle_write(const boost::system::error_code &ec) {
    ...
    if (!ec) {
      ...
      CYNG_LOG_ERROR(logger_, "[broker] " << target_
