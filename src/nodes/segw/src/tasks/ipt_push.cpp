...
namespace smf {
    ipt_push::ipt_push(cyng::channel_weak wp
        ...
        , std::uint8_t nr)
		: sigs_{
            std::bind(&ipt_push::init, this),	//	0
            std::bind(&ipt_push::run, this),	//	1
            std::bind(
                &ipt_push::on_channel_open,
                this,
                std::placeholders::_1,
                std::placeholders::_2,
                std::placeholders::_3,
            ...
            std::bind(
                &ipt_push::on_channel_close,
                this,
                std::placeholders::_1,
                std::placeholders::_2), // "on.channel.close"
                std::bind(&ipt_push::stop, this, std::placeholders::_1)	
		}	
		, channel_(wp)
		, logger_(logger)
        , bus_(bus)
    ...
    void ipt_push::stop(cyng::eod) { CYNG_LOG_INFO(logger_, "[push] stopped"); }
    ...
    bool ipt_push::open_channel(ipt::push_channel &&pc) { return bus_.open_channel(pc, channel_); }
    ...
    cyng::tuple_t ipt_push::convert_to_payload(
        cyng::table const *tbl_mirror_data,
        cyng::key_t const &pk,
        std::uint32_t ops,
        cyng::obis profile,
    ...
    cyng::obis_path_t ipt_push::get_registers(cyng::table const *tbl) {
    ...
    void ipt_push::on_channel_close(bool success, std::uint32_t channel) {
        if (success) {
