...
namespace smf
{
	CP210x::CP210x(std::weak_ptr<cyng::channel> wp
		, cyng::controller& ctl
		, cyng::logger logger)
		: sigs_{
			std::bind(&CP210x::stop, this, std::placeholders::_1),
			std::bind(&CP210x::receive, this, std::placeholders::_1),
			std::bind(&CP210x::reset_target_channels, this, std::placeholders::_1)
	}, channel_(wp)
		, logger_(logger)
		, ctl_(ctl)
		, parser_([this](hci::msg const& msg) {
			this->put(msg);
		})
		, targets_()
	{
	...
	void CP210x::stop(cyng::eod) {
		CYNG_LOG_INFO(logger_, "CP210x stopped");
	}
