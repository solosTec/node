...
namespace smf {
  namespace rdr {
    ...
    server::server(cyng::controller &ctl, cfg &c, cyng::logger logger,
                   lmn_type type)
        : ctl_(ctl), cfg_(c), logger_(logger), type_(type),
      ...
      acceptor_.async_accept([this](boost::system::error_code ec,
        ...
        if (!ec) {
          CYNG_LOG_INFO(logger_,
                        "[RDR] new session " << socket.remote_endpoint());

          auto sp = std::shared_ptr<session>(
              new session(std::move(socket), registry_, cfg_, logger_, type_),
              [this](session *s) {
                //
                ...
                --session_counter_;
                CYNG_LOG_TRACE(
                    logger_, "[RDR] session(s) running: " << session_counter_);
