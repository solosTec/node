...
#define DEFINE_OBIS(p1, p2, p3, p4, p5, p6) cyng::obis(0x##p1, 0x##p2, 0x##p3, 0x##p4, 0x##p5, 0x##p6)
...
class obis_ctx {
  public:
    ...
    char const *get_desc() const { return desc_; }
    std::size_t get_type() const { return type_; }
...
class edis_ctx {
  public:
    edis_ctx(char const *s, char const *name, char const *unit)
        ...
        , name_(name)
        , unit_(unit) {}
    char const *get_short() const { return short_; }
    char const *get_name() const { return name_; }
    char const *get_unit() const { return unit_; }
...
int main(int argc, char **argv) {
    ...
    static edis_map_t edis_map{
        ...
        {cyng::edis(0, 8, 0),
         {"REG_DP2_DEMAND_INTERVAL", "Duration of measurement interval for current power value (1-0.0:8.0*255)", "min"}},
    ...
    obis_map_t obis_map{
        ...
        {DEFINE_OBIS(00, 00, 60, 02, 01, ff),
         {"DATE_TIME_PARAMETERISATION", cyng::TC_NULL, "Date of last parameterisation (00-03-26)"}},
        ...
        {DEFINE_OBIS(81, 00, 00, 09, 0b, 01),
         {"TZ_OFFSET", cyng::TC_UINT16, "offset to actual time zone in minutes (-720 .. +720)"}},
        ...
        {DEFINE_OBIS(81, 02, 00, 07, 00, ff),
         {"ROOT_CUSTOM_INTERFACE",
          cyng::TC_NULL,
          "see: 7.3.1.3 Datenstruktur zum Lesen / Setzen der Parameter für die Kundenschnittstelle"}},
        ...
        {DEFINE_OBIS(81, 02, 00, 07, 10, ff),
         {"ROOT_CUSTOM_PARAM",
          cyng::TC_NULL,
          "see: 7.3.1.4 Datenstruktur für dynamischen Eigenschaften der Endkundenschnittstelle "}},
        {DEFINE_OBIS(81, 02, 17, 07, 00, 00),
         {"CUSTOM_IF_IP_CURRENT_1", cyng::TC_IP_ADDRESS, "current IP address (customer interface)"}},
        ...
        {DEFINE_OBIS(81, 04, 00, 06, 00, ff),
         {"ROOT_WAN", cyng::TC_NULL, "see: 7.3.1.5 Datenstruktur zur Abfrage des WAN Status "}},
        {DEFINE_OBIS(81, 04, 00, 07, 00, ff),
         {"ROOT_WAN_PARAM", cyng::TC_NULL, "see: 7.3.1.6 Datenstruktur zum Lesen/Setzen der WAN Parameter "}},
        {DEFINE_OBIS(81, 04, 02, 07, 00, ff),
         {"ROOT_GSM", cyng::TC_NULL, "see: Datenstruktur zum Lesen/Setzen der GSM Parameter "}},
        {DEFINE_OBIS(81, 04, 0d, 06, 00, 00),
         {"CLASS_OP_LOG_PROVIDER", cyng::TC_UINT32, "	aktueller Provider-Identifier(uint32)"}},
        {DEFINE_OBIS(81, 04, 0d, 06, 00, ff), {"GSM_ADMISSIBLE_OPERATOR", cyng::TC_NULL, ""}},
        {DEFINE_OBIS(81, 04, 0d, 07, 00, ff),
         {"ROOT_GPRS_PARAM", cyng::TC_NULL, "see: Datenstruktur zum Lesen / Setzen der Provider-abhängigen GPRS-Parameter "}},
        ...
        {DEFINE_OBIS(81, 04, 62, 3C, 01, 01), {"PPPoE_USERNAME", cyng::TC_STRING, "PPPoE username"}},
        {DEFINE_OBIS(81, 04, 62, 3C, 02, 01), {"PPPoE_PASSWORD", cyng::TC_STRING, "PPPoE passphrase"}},
        {DEFINE_OBIS(81, 04, 62, 3C, 03, 01),
         {"PPPoE_MODE",
          cyng::TC_UINT8,
          "PPPoE mode"}}, // Wird dieser Parameter geschrieben, darf ein KM-Modul einen Reboot-Vorgang durchführen.
        ...
        {DEFINE_OBIS(81, 06, 64, 3c, 01, 04),
         {"W_MBUS_MAX_SML_TIMEOUT_IN",
          cyng::TC_UINT16,
          "max timeout between SML close request and SML open response from device to gateway"}},
        {DEFINE_OBIS(81, 06, 64, 3c, 01, 05),
         {"W_MBUS_MAX_SML_TIMEOUT_OUT",
          cyng::TC_UINT16,
          "max timeout between SML close request and SML open response from gateway to device"}},
        ...
        {DEFINE_OBIS(81, 48, 00, 32, 01, 01), {"LAN_PPPoE_ENABLED", cyng::TC_BOOL, ""}},
        {DEFINE_OBIS(81, 48, 00, 32, 03, 01), {"LAN_DSL_ENABLED", cyng::TC_BOOL, "true = DLS, false = LAN"}},
        {DEFINE_OBIS(81, 48, 0d, 06, 00, ff),
         {"ROOT_LAN_DSL", cyng::TC_NULL, "see: 7.3.1.19 Datenstruktur zur Abfrage dynamischer LAN/DSL- Betriebsparameter"}},
        {DEFINE_OBIS(81, 48, 17, 07, 00, 00), {"CODE_IF_LAN_ADDRESS", cyng::TC_IP_ADDRESS, "IPv4 or IPv6 address"}},
        {DEFINE_OBIS(81, 48, 17, 07, 00, 01), {"CODE_IF_DSL_ADDRESS", cyng::TC_IP_ADDRESS, "IPv4 or IPv6 address"}},
        {DEFINE_OBIS(81, 48, 17, 07, 00, ff),
         {"IF_LAN_DSL", cyng::TC_NULL, "see: 7.3.1.18 Datenstruktur zum Lesen / Setzen der LAN/DSL-Parameter"}},
        ...
        {DEFINE_OBIS(81, 48, 17, 07, 04, 01), {"CODE_IF_LAN_DSL_PRIMARY", cyng::TC_IP_ADDRESS, "set/get OBIS_IF_LAN_DSL"}},
        {DEFINE_OBIS(81, 48, 17, 07, 05, 00), {"CODE_IF_LAN_DNS_SECONDARY", cyng::TC_IP_ADDRESS, ""}},
        {DEFINE_OBIS(81, 48, 17, 07, 05, 01), {"CODE_IF_LAN_DSL_SECONDARY", cyng::TC_IP_ADDRESS, "set/get OBIS_IF_LAN_DSL"}},
        {DEFINE_OBIS(81, 48, 17, 07, 06, 00), {"CODE_IF_LAN_DNS_TERTIARY", cyng::TC_IP_ADDRESS, ""}},
        {DEFINE_OBIS(81, 48, 17, 07, 06, 01), {"CODE_IF_LAN_DLS_TERTIARY", cyng::TC_IP_ADDRESS, "set/get OBIS_IF_LAN_DSL"}},
        ...
        {DEFINE_OBIS(81, 48, 31, 32, 07, 01), {"TCP_REPLY_ICMP", cyng::TC_BOOL, "reply to received ICMP packages"}},
        ...
        {DEFINE_OBIS(81, 49, 00, 00, 10, ff),
         {"PUSH_SERVICE", cyng::TC_NULL, "options are PUSH_SERVICE_IPT, PUSH_SERVICE_SML or PUSH_SERVICE_KNX"}},
        {DEFINE_OBIS(81, 49, 0d, 06, 00, ff),
         {"ROOT_IPT_STATE", cyng::TC_NULL, "see: 7.3.1.8 Datenstruktur zur Abfrage des IPT Status "}},
        {DEFINE_OBIS(81, 49, 0d, 07, 00, ff),
         {"ROOT_IPT_PARAM", cyng::TC_NULL, "see: 7.3.1.9 Datenstruktur zur Lesen/Setzen der IPT Parameter "}},
        ...
        {DEFINE_OBIS(81, 81, 00, 02, 00, 03),
         {"MSG_COUNTER", cyng::TC_UINT32, "Anzahl aller Nachrichten zur Übertragung des Binary"}},
        {DEFINE_OBIS(81, 81, 00, 02, 00, 04),
         {"LAST_MSG", cyng::TC_UINT32, "Nummer der zuletzt erfolgreich übertragenen Nachricht des	Binary"}},
        ...
        {DEFINE_OBIS(81, 81, 10, 06, 01, ff), {"CODE_LIST_1_VISIBLE_DEVICES", cyng::TC_TUPLE, "1. Liste der sichtbaren Sensoren"}},
        {DEFINE_OBIS(81, 81, 10, 06, 02, ff), {"CODE_LIST_2_VISIBLE_DEVICES", cyng::TC_TUPLE, "2. Liste der sichtbaren Sensoren"}},
        {DEFINE_OBIS(81, 81, 10, 06, ff, ff),
         {"ROOT_VISIBLE_DEVICES", cyng::TC_NULL, "visible devices (Liste der sichtbaren Sensoren)"}},
        ...
        {DEFINE_OBIS(81, 81, 11, 06, 01, ff), {"CODE_LIST_1_ACTIVE_DEVICES", cyng::TC_TUPLE, "1. Liste der aktiven Sensoren)"}},
        {DEFINE_OBIS(81, 81, 11, 06, 02, ff), {"CODE_LIST_2_ACTIVE_DEVICES", cyng::TC_TUPLE, "2. Liste der aktiven Sensoren)"}},
        ...
        {DEFINE_OBIS(81, 81, 11, 06, ff, ff),
         {"ROOT_ACTIVE_DEVICES", cyng::TC_NULL, "active devices (Liste der aktiven Sensoren)"}},
        {DEFINE_OBIS(81, 81, 12, 06, ff, ff), {"ROOT_DEVICE_INFO", cyng::TC_NULL, "extended device information"}},
        {DEFINE_OBIS(81, 81, 27, 32, 07, 01),
         {"OBISLOG_INTERVAL",
          cyng::TC_UINT16,
        ...
        {DEFINE_OBIS(81, 81, 81, 60, ff, ff),
         {"ROOT_ACCESS_RIGHTS", cyng::TC_NULL, "see: 7.3.1.2 Datenstruktur zur Parametrierung der Rollen / Benutzerrechte "}},
        ...
        {DEFINE_OBIS(81, 81, c7, 81, 01, ff),
         {"ROOT_FILE_TRANSFER", cyng::TC_NULL, "7.3.2.28 Datenstruktur zum remote Firmware-/Datei-Download (Übertragung) "}},
        ...
        {DEFINE_OBIS(81, 81, c7, 82, 01, ff),
         {"ROOT_DEVICE_IDENT",
          cyng::TC_NULL,
          "see: 7.3.2.9 Datenstruktur zur Abfrage der Geräte-Identifikation: firmware, file, application) "}},
        ...
        {DEFINE_OBIS(81, 81, c7, 82, 09, ff),
         {"HARDWARE_FEATURES", cyng::TC_NULL, "hardware equipment (charge, type, ...) 81 81 C7 82 0A NN"}},
        {DEFINE_OBIS(81, 81, c7, 82, 0A, 01), {"DEVICE_MODEL", cyng::TC_STRING, "model code (VMET-1KW-221-1F0)"}},
        {DEFINE_OBIS(81, 81, c7, 82, 0A, 02), {"DEVICE_SERIAL", cyng::TC_STRING, "serial number (3894517)"}},
        ...
        {DEFINE_OBIS(81, 81, c7, 86, 00, ff),
         {"ROOT_SENSOR_PARAMS", cyng::TC_NULL, "data mirror root element (Eigenschaften eines Datenspiegels)"}},
        {DEFINE_OBIS(81, 81, c7, 86, 01, ff),
         {"ROOT_SENSOR_BITMASK", cyng::TC_UINT16, " Bitmask to define bits that will be transferred into log"}},
        {DEFINE_OBIS(81, 81, c7, 86, 02, ff),
         {"AVERAGE_TIME_MS", cyng::TC_NULL, "average time between two received data records (milliseconds)"}},
        {DEFINE_OBIS(81, 81, c7, 86, 03, ff), {"DATA_AES_KEY", cyng::TC_NULL, ""}},
        {DEFINE_OBIS(81, 81, c7, 86, 04, ff),
         {"TIME_REFERENCE", cyng::TC_UINT8, "0 == UTC, 1 == UTC + time zone, 2 == local time"}},
        ...
        {DEFINE_OBIS(81, 81, c7, 86, 18, ff),
         {"PROFILE_INITIAL", cyng::TC_NULL, "81, 81, C7, 86, 18, NN with NN = 01 .. 0A for open registration periods"}},
        {DEFINE_OBIS(81, 81, c7, 86, 20, ff),
        ...
        {DEFINE_OBIS(81, 81, c7, 88, 02, ff), {"NTP_SERVER", cyng::TC_NULL, "List of NTP servers"}},
        {DEFINE_OBIS(81, 81, c7, 88, 03, ff), {"NTP_PORT", cyng::TC_UINT16, "NTP port (123)"}},
        {DEFINE_OBIS(81, 81, c7, 88, 04, ff), {"NTP_TZ", cyng::TC_UINT32, "timezone"}},
        {DEFINE_OBIS(81, 81, c7, 88, 05, ff),
         {"CODE_NTP_OFFSET", cyng::TC_NULL, "Offset to transmission of the signal for synchronization"}},
        {DEFINE_OBIS(81, 81, c7, 88, 06, ff), {"NTP_ACTIVE", cyng::TC_NULL, "NTP enabled/disables"}},
        ...
        {DEFINE_OBIS(81, 81, c7, 8a, 04, ff),
         {"PUSH_SOURCE", cyng::TC_NULL, "options are PUSH_SOURCE_PROFILE, PUSH_SOURCE_INSTALL and PUSH_SOURCE_SENSOR_LIST"}},
        {DEFINE_OBIS(81, 81, c7, 8a, 21, ff),
         {"PUSH_SERVICE_IPT", cyng::TC_NULL, "SML data response without request - typical IP - T push"}},
        {DEFINE_OBIS(81, 81, c7, 8a, 22, ff),
         {"PUSH_SERVICE_SML", cyng::TC_NULL, "SML data response without request - target is SML client address"}},
        ...
        {DEFINE_OBIS(81, 81, c7, 93, 01, ff),
         {"IF_1107_ACTIVE", cyng::TC_BOOL, " if true 1107 interface active otherwise SML interface active"}},
        ...
        {DEFINE_OBIS(81, 81, c7, c7, fd, 01), {"ATTENTION_JOB_IS_RUNNINNG", cyng::TC_NULL, "attention: job is running"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 00), {"ATTENTION_UNKNOWN_ERROR", cyng::TC_NULL, "attention: unknown error"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 01), {"ATTENTION_UNKNOWN_SML_ID", cyng::TC_NULL, "attention: unknown SML ID"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 02), {"ATTENTION_NOT_AUTHORIZED", cyng::TC_NULL, "attention: not authorized"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 03),
         {"ATTENTION_NO_SERVER_ID", cyng::TC_NULL, "attention: unable to find recipient for request"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 04), {"ATTENTION_NO_REQ_FIELD", cyng::TC_NULL, "attention: no request field"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 05), {"ATTENTION_CANNOT_WRITE", cyng::TC_NULL, "attention: cannot write"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 06), {"ATTENTION_CANNOT_READ", cyng::TC_NULL, "attention: cannot read"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 07), {"ATTENTION_COMM_ERROR", cyng::TC_NULL, "attention: communication error"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 08), {"ATTENTION_PARSER_ERROR", cyng::TC_NULL, "attention: parser error"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 09), {"ATTENTION_OUT_OF_RANGE", cyng::TC_NULL, "attention: out of range"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 0a), {"ATTENTION_NOT_EXECUTED", cyng::TC_NULL, "attention: not executed"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 0b), {"ATTENTION_INVALID_CRC", cyng::TC_NULL, "attention: invalid CRC"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 0c), {"ATTENTION_NO_BROADCAST", cyng::TC_NULL, "attention: no broadcast"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 0d), {"ATTENTION_UNEXPECTED_MSG", cyng::TC_NULL, "attention: unexpected message"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 0e), {"ATTENTION_UNKNOWN_OBIS_CODE", cyng::TC_NULL, "attention: unknown OBIS code"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 0f),
         {"ATTENTION_UNSUPPORTED_DATA_TYPE", cyng::TC_NULL, "attention: data type not supported"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 10),
         {"ATTENTION_ELEMENT_NOT_OPTIONAL", cyng::TC_NULL, "attention: element is not optional"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 11), {"ATTENTION_NO_ENTRIES", cyng::TC_NULL, "attention: no entries"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 12),
         {"ATTENTION_END_LIMIT_BEFORE_START", cyng::TC_NULL, "attention: end limit before start"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 13),
         {"ATTENTION_NO_ENTRIES_IN_RANGE", cyng::TC_NULL, "attention: range is empty - not the profile"}},
        {DEFINE_OBIS(81, 81, c7, c7, fe, 14), {"ATTENTION_MISSING_CLOSE_MSG", cyng::TC_NULL, "attention: missing close message"}},
    ...
    if (ofs.is_open()) {
        ...
        if (std::strftime(buffer, sizeof(buffer), "%F %H:%M:%S", &buf)) {
            ofs << "\t// generated at " << buffer << std::endl;
        }

        ofs << "\t// " << obis_map.size() << " OBIS codes (" << (obis_map.size() * 6u) << " Bytes)" << std::endl << std::endl;
        ...
        for (auto const &ctx : obis_map) {
            ...
            ++counter;
            std::cout << counter << " - " << cyng::traits::names[ctx.second.get_type()] << " - " << ctx.second.get_name() << " "
                      << ctx.second.get_desc() << std::endl;
            ...
            ofs << "\tOBIS_CODE_DEFINITION(" << std::setw(2) << next_group << ", " << std::setw(2)
                << static_cast<unsigned>(ctx.first[cyng::obis::value_group::VG_CHANNEL]) << ", " << std::setw(2)
                << static_cast<unsigned>(ctx.first[cyng::obis::value_group::VG_INDICATOR]) << ", " << std::setw(2)
                << static_cast<unsigned>(ctx.first[cyng::obis::value_group::VG_MODE]) << ", " << std::setw(2)
                << static_cast<unsigned>(ctx.first[cyng::obis::value_group::VG_QUANTITY]) << ", " << std::setw(2)
                << static_cast<unsigned>(ctx.first[cyng::obis::value_group::VG_STORAGE]) << ", " << ctx.second.get_name() << ");";

            if (ctx.second.get_type() != cyng::TC_NULL) {
                ofs << "\t// " << cyng::traits::names[ctx.second.get_type()];
            }
    ...
    if (ofs2.is_open()) {
        ...
        for (auto const &ctx : obis_map) {
            ...
            ofs2 << "case 0x" << ctx.first.to_uint64() << ": return \"" << ctx.second.get_name() << "\";";
    ...
    std::filesystem::path p3 = std::filesystem::path(__OUTPUT_PATH).replace_filename("descr.ipp");
    std::ofstream ofs3(p3.string(), std::ios::trunc);
    if (ofs3.is_open()) {
        ...
        for (auto const &ctx : obis_map) {
            ...
            ofs3 << "case 0x" << ctx.first.to_uint64() << ": return \"" << ctx.second.get_desc() << "\";";
