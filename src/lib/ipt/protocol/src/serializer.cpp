...
namespace smf {
    ...
    namespace ipt {
        ...
        cyng::buffer_t serializer::req_login_scrambled(std::string const &name, std::string const &pwd, scramble_key const key) {
        ...
        //
        std::pair<cyng::buffer_t, sequence_t> serializer::req_open_push_channel(
            ...
            std::size_t const length = target.size() + account.size() + number.size() + version.size() + id.size() +
                                       sizeof(timeout) + 5; //	one \0 for each string

            auto deq = write_header(code::TP_REQ_OPEN_PUSH_CHANNEL, last_seq_, length);
        ...
        //
        cyng::buffer_t serializer::res_open_push_channel(
            sequence_t seq,
            response_t res,
...
#ifdef _DEBUG
            static_assert(
                17 == sizeof(res) + sizeof(channel) + sizeof(source) + sizeof(packet_size) + sizeof(window_size) + sizeof(status) +
                          sizeof(count),
                "res_open_push_channel(length assumption invalid)");
#endif
        ...
        //
        cyng::buffer_t serializer::req_transfer_push_data(
            std::uint32_t channel,
            std::uint32_t source,
            std::uint8_t status,
            std::uint8_t block,
            ...
            last_seq_ = sgen_();
            auto deq = write_header(
                code::TP_REQ_PUSHDATA_TRANSFER,
                last_seq_,
        ...
        //
        cyng::buffer_t serializer::res_transfer_push_data(
            sequence_t seq,
            response_t res,
            std::uint32_t channel,
            std::uint32_t source,
            ...
            std::uint8_t block) {
            static_assert(
                11 == sizeof(res) + sizeof(channel) + sizeof(source) + sizeof(status) + sizeof(block),
                "res_transfer_push_data(length assumption invalid)");
        ...
        //
        cyng::buffer_t serializer::res_close_connection(sequence_t seq, response_t res) {
        ...
        //
        cyng::buffer_t serializer::res_protocol_version(sequence_t seq, response_t res) {
        ...
        //
        cyng::buffer_t serializer::res_network_status(
            sequence_t seq,
            std::uint8_t dev,
            std::uint32_t stat_1,
            std::uint32_t stat_2,
            ...
            std::string imei) {
            auto deq = write_header(
                code::APP_RES_NETWORK_STATUS, seq, sizeof(dev) + (sizeof(std::uint32_t) * 5) + imsi.size() + imei.size() + 2);
        ...
        //
        cyng::buffer_t serializer::res_ip_statistics(sequence_t seq, response_t res, std::uint64_t rx, std::uint64_t sx) {
        ...
        //
        cyng::buffer_t serializer::res_device_auth(
            sequence_t seq,
            std::string account,
            std::string password,
            std::string number,
            ...
            auto deq = write_header(
                code::APP_RES_DEVICE_AUTHENTIFICATION,
                seq,
                account.size() + password.size() + number.size() + description.size() + 4);
            deq.push_back(write(account));
        ...
        //
        //			last_seq_ = sgen_();
        //			write_header(code::APP_REQ_TRACEROUTE, last_seq_, sizeof(ipt_link) + sizeof(traceroute_idx) +
        // sizeof(hop_counter) + sizeof(channel));
        //
        ...
        serializer::req_register_push_target(std::string target, std::uint16_t p_size, std::uint8_t w_size) {
            last_seq_ = sgen_();
        ...
        //
        cyng::buffer_t serializer::res_register_push_target(sequence_t seq, response_t res, std::uint32_t channel) {
        ...
        //
        cyng::buffer_t serializer::res_deregister_push_target(sequence_t seq, response_t res, std::string target) {
        ...
        //		}
        //
        cyng::buffer_t serializer::res_unknown_command(sequence_t seq, command_t cmd) {
        ...
        std::deque<cyng::buffer_t> serializer::write_header(code cmd, sequence_t seq, std::size_t length) {
            ...
            case code::CTRL_RES_LOGIN_SCRAMBLED: //	login response
            case code::CTRL_REQ_LOGIN_PUBLIC:
            case code::CTRL_REQ_LOGIN_SCRAMBLED: //	login request
                break;
            default:
        ...
        cyng::buffer_t serializer::write(scramble_key::key_type const &key) {
            ...
            std::transform(std::begin(key), std::end(key), std::back_inserter(buffer), [this](char c) { return scrambler_[c]; });
        ...
        cyng::buffer_t serializer::scramble(cyng::buffer_t &&data) {
            ...
            std::transform(std::begin(data), std::end(data), std::begin(data), [this](char c) { return scrambler_[c]; });
            return data;
        }
        ...
        std::size_t serializer::get_scrambler_index() const { return scrambler_.get_index(); }
