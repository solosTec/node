...
namespace smf {
  namespace ipt {
    bus::bus(boost::asio::io_context &ctx, cyng::logger logger,
             toggle::server_vec_t &&tgl, std::string model,
             parser::command_cb cb_cmd, parser::data_cb cb_stream,
             auth_cb cb_auth)
        : state_(state::START), ctx_(ctx), logger_(logger),
          tgl_(std::move(tgl)), model_(model), cb_cmd_(cb_cmd),
          cb_auth_(cb_auth), endpoints_(), socket_(ctx), timer_(ctx),
          dispatcher_(ctx), serializer_(tgl_.get().sk_),
          parser_(tgl_.get().sk_,
                  std::bind(&bus::cmd_complete, this, std::placeholders::_1,
                            std::placeholders::_2),
                  cb_stream),
          buffer_write_(), input_buffer_(), registrant_(), targets_() {}
    ...
    void bus::start() {
      ...
      try {
        ...
        CYNG_LOG_WARNING(logger_,
                         "[ipt] connect failed - switch to " << tgl_.get());
    ...
    void bus::set_reconnect_timer(std::chrono::seconds delay) {
      ...
      if (!is_stopped()) {
        CYNG_LOG_TRACE(logger_, "[ipt] set reconnect timer: " << delay.count()
                                                              << " seconds");
        timer_.expires_after(delay);
        timer_.async_wait(boost::asio::bind_executor(
            dispatcher_, boost::bind(&bus::reconnect_timeout, this,
    ...
    void bus::start_connect(
        boost::asio::ip::tcp::resolver::results_type::iterator endpoint_iter) {
      if (endpoint_iter != endpoints_.end()) {

        CYNG_LOG_INFO(logger_,
                      "[ipt] connect to " << endpoint_iter->endpoint());
    ...
    void bus::handle_connect(
      ...
      if (!socket_.is_open()) {
        ...
        CYNG_LOG_WARNING(logger_,
                         "ipt [" << tgl_.get() << "] connect timed out");
      ...
      // Check if the connect operation failed before the deadline expired.
        ...
        CYNG_LOG_WARNING(logger_, "ipt [" << tgl_.get() << "] connect error: "
                                          << ec.message());
      ...
      // Otherwise we have successfully established a connection.
        ...
        CYNG_LOG_INFO(logger_, "ipt [" << tgl_.get() << "] connected to "
    ...
    void bus::do_write() {
      ...
      CYNG_LOG_TRACE(logger_, "ipt [" << tgl_.get() << "] write: "
                                      << buffer_write_.front().size()
                                      << " bytes");

      // Start an asynchronous operation to send a heartbeat message.
      boost::asio::async_write(
          socket_,
          boost::asio::buffer(buffer_write_.front().data(),
                              buffer_write_.front().size()),
          dispatcher_.wrap(
    ...
    void bus::handle_read(const boost::system::error_code &ec, std::size_t n) {
      ...
      if (!ec) {
        ...
        CYNG_LOG_ERROR(logger_, "ipt [" << tgl_.get() << "] on receive "
                                        << ec.value() << ": s" << ec.message());
    ...
    void bus::handle_write(const boost::system::error_code &ec) {
      ...
      if (!ec) {
        ...
        CYNG_LOG_ERROR(logger_, "ipt [" << tgl_.get()
                                        << "] on heartbeat: " << ec.message());
    ...
    void bus::cmd_complete(header const &h, cyng::buffer_t &&body) {
      CYNG_LOG_TRACE(logger_, "ipt [" << tgl_.get() << "] cmd "
                                      << command_name(h.command_));
      ...
      case code::APP_REQ_DEVICE_AUTHENTIFICATION:
        send(serializer_.res_device_auth(h.sequence_, tgl_.get().account_,
                                         tgl_.get().pwd_,
    ...
    void bus::res_login(cyng::buffer_t &&data) {
      ...
      if (r.is_success()) {
        ...
        CYNG_LOG_INFO(logger_,
                      "ipt [" << tgl_.get() << "] " << r.get_response_name());
        ...
        //
        if (watchdog != 0) {
          CYNG_LOG_INFO(logger_, "ipt [" << tgl_.get() << "] set watchdog: "
                                         << watchdog << " minutes");
        ...
        CYNG_LOG_WARNING(logger_, "ipt [" << tgl_.get() << "] login failed: "
                                          << r.get_response_name());
        boost::system::error_code ignored_ec;
        socket_.close(ignored_ec);
      }
    ...
    void bus::open_channel(push_channel pcc) {
      ...
      boost::asio::post(dispatcher_, [this, pcc]() {
        bool const b = buffer_write_.empty();
        auto r = serializer_.req_open_push_channel(pcc.target_, pcc.account_,
                                                   pcc.number_, pcc.version_,
                                                   pcc.id_, pcc.timeout_);
    ...
    void bus::res_register_target(header const &h, cyng::buffer_t &&body) {
      ...
      //
      boost::asio::post(
          dispatcher_, [this, h, res, channel]() {
            ...
            if (pos != registrant_.end()) {
              ...
              CYNG_LOG_ERROR(logger_, "[ipt] cmd "
                                          << ipt::command_name(h.command_)
                                          << ": missing registrant");
            }
          });
    ...
    void bus::pushdata_transfer(header const &h, cyng::buffer_t &&body) {
       ...
       */
      auto [channel, source, status, block, data] =
          tp_req_pushdata_transfer(std::move(body));

      //
      //	sync with strand
      //
      boost::asio::post(dispatcher_, [this, h, channel, source, status, block,
                                      data]() {
    ...
    void bus::open_connection(std::string number, sequence_t seq) {
      ...
      if (state_ == state::AUTHORIZED) {
        ...
        send(serializer_.res_open_connection(
            seq, tp_res_open_connection_policy::DIALUP_SUCCESS));
      } else {
        send(serializer_.res_open_connection(
            seq, tp_res_open_connection_policy::BUSY));
      }
    }
    ...
    void bus::close_connection(sequence_t seq) {
      if (state_ == state::LINKED) {
        state_ = state::AUTHORIZED;
        send(serializer_.res_close_connection(
            seq,
            tp_res_close_connection_policy::CONNECTION_CLEARING_SUCCEEDED));
        CYNG_LOG_TRACE(logger_, "[ipt] unlinked");
      } else {
        send(serializer_.res_close_connection(
            seq, tp_res_close_connection_policy::CONNECTION_CLEARING_FAILED));
      }
    }
