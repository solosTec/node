...
namespace smf
{
	namespace mbus
	{
		...
		bool vif::is_date() const {
			return ((value_ & 0x7E) == 0x6C)
				&& ((value_ & 0x01) == 0)
				;
		}

		bool vif::is_time() const {
			return ((value_ & 0x7E) == 0x6C)
				&& ((value_ & 0x01) == 1)
				;
		}
		...
		std::pair<std::int8_t, unit> vif::get_vib_type() const {
			...
			//
				...
				//	E110 1101
				//	DT01: Extended date and time Time and date to sec.data field = 0110b, type I point(actual or associated with a storage number / function)
