...
namespace smf {
	namespace sml {
		...
		namespace detail
		{
			...
			cyng::tuple_t factory_policy<cyng::buffer_t>::create(cyng::buffer_t const& v) {
				return cyng::make_tuple(static_cast<std::uint8_t>(PROC_PAR_VALUE), v);
			}
			...
			cyng::tuple_t factory_policy<boost::asio::ip::address>::create(boost::asio::ip::address v) 	{
				return (v.is_v4())
					? factory_policy<boost::asio::ip::address_v4>::create(v.to_v4())
		...
		cyng::tuple_t make_local_timestamp(std::chrono::system_clock::time_point tp)
		{
			...
			return cyng::make_tuple(static_cast<std::uint8_t>(PROC_PAR_TUPELENTRY)
				, cyng::make_tuple(static_cast<std::uint32_t>(ut)
		...
		{
			return cyng::make_tuple(static_cast<std::uint8_t>(PROC_PAR_VALUE), cyng::null());
		}
