...
namespace smf {
    namespace sml {
        ...
        bool parser::is_closed() const {
            return (last_type_ == msg_type::CLOSE_REQUEST) || (last_type_ == msg_type::CLOSE_RESPONSE);
        }
            ...
            if (!stack_.empty()) {
                ...
                if (stack_.top().values_.size() == 5) {
                    ...
                    if ((choice.size() == 2)) {
                        ...
                        //
                        //	callback
                        //	std::function<void(std::string, std::uint8_t, std::uint8_t, msg_type, cyng::tuple_t,
                        // std::uint16_t)>;
                        //
        ...
        std::size_t parser::list::pos() const { return size_ - values_.size(); }
