...
namespace smf {
	namespace sml {
		...
		cyng::object customize_value(cyng::obis code, cyng::object obj)
		{
			if (code.starts_with(cyng::make_buffer({ 0x81, 0x49, 0x17, 0x07, 0x00 }))
			...
			else if (cyng::compare_n(code, OBIS_TARGET_PORT, 5)	//	TARGET_PORT
			...
			else if (code.starts_with(cyng::make_buffer({ 0x81, 0x49, 0x63, 0x3C, 0x01 }))	//	IP-T user name redundancy 1
				...
				|| code.starts_with(cyng::make_buffer({ 0x81, 0x81, 0xC7, 0x82, 0x0A }))
		...
		cyng::obis_path_t read_param_tree_path(cyng::object obj) {
			...
			std::transform(tpl.begin(), tpl.end(), std::back_inserter(result), [](cyng::object obj) -> cyng::obis {
		...
		{
			return cyng::make_object(to_string(obj));
		}
