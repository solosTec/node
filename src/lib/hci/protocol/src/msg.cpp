...
namespace smf {
    namespace hci {
        ...
        std::uint32_t msg::get_time_stamp() const { return (has_timestamp()) ? cyng::to_numeric<std::uint32_t>(time_stamp_) : 0u; }

        std::int16_t msg::get_crc() const { return (has_crc16()) ? cyng::to_numeric<std::int16_t>(fcs_) : 0u; }

        std::ostream &operator<<(std::ostream &os, msg const &m) {
            os << +m.get_length()                      //  message size
               << " bytes, ep: " << +m.header_ctrl_ep_ //  Control and Endpoint field
               << ", id: " << +m.header_msg_id_        //  message ID field
                ;
            return os;
        }
