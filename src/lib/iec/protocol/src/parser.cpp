...
namespace smf
{
	namespace iec
	{
		...
		cyng::obis to_obis(std::string code, std::uint8_t medium) {
			...
			//	auto const code = vec.at(0);
				...
				if (edis_code.size() == 3) {
					...
					return (c == 0)
						? cyng::make_obis(0, 0, c, d, e, f)
						: cyng::make_obis(medium, 0, c, d, e, f)
						;
		...
		std::tuple<std::uint8_t, std::uint8_t, std::uint8_t, std::uint8_t> to_u8(std::string c, std::string d, std::string e, std::string f) {
			try {
				return {
					static_cast<std::int8_t>(std::stoi(c)),
					static_cast<std::int8_t>(std::stoi(d)),
