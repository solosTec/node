...
BOOST_AUTO_TEST_CASE(parser) {
    ...
    auto const inp = cyng::make_buffer(
        {0xce, 0x44, 0xa8, 0x15, 0x74,
         0x31, 0x45, 0x04, 0x01, 0x02,
         0x7f, // header complete
         0x46, 0x00, 0xc0, 0x05, /* data */ 
    ...
    smf::mbus::radio::parser p(
        [&](smf::mbus::radio::header const &h, smf::mbus::radio::tplayer const &t, cyng::buffer_t const &payload) {
            ...
            //  EMH
            BOOST_REQUIRE_EQUAL(mc.first, static_cast<char>(0xa8));
...
BOOST_AUTO_TEST_CASE(els) {
    //  This test doesn't working yet. One reason is it's unclaer what the correct AES kex is.
    // https://platform.lobaro.com/#/wmbus/parser
    //  Elster (ELS), Gas 0x03, LinkLayer
    auto const inp = cyng::make_buffer({0x2e, 0x44, 0x93, 0x15, 0x78, 0x56, 0x34, 0x12, 0x33, 0x03, // header
                                        0x33, 0x63, 0x7a, 0x2a, 0x00, 0x20, 0x05, 0x59, 0x23, 0xc9, 0x5a, 0xaa, 0x26, 0xd1, 0xb2,
                                        0xe7, 0x49, 0x3b, 0x2a, 0x8b, 0x01, 0x3e, 0xc4, 0xa6, 0xf6, 0xd3, 0x52, 0x9b, 0x52, 0x0e,
                                        0xdf, 0xf0, 0xea, 0x6d, 0xef, 0xc9, 0x55, 0xb2, 0x9d, 0x6d, 0x69, 0xeb, 0xf3, 0xec, 0x8a});
    ...
    smf::mbus::radio::parser p(
        [&](smf::mbus::radio::header const &h, smf::mbus::radio::tplayer const &t, cyng::buffer_t const &payload) {
            ...
            // auto const aes = cyng::to_aes_key<cyng::crypto::aes128_size>("0102030405060708090A0B0C0D0E0F11");
            auto const aes = cyng::to_aes_key<cyng::crypto::aes128_size>("00000000000000000000000000000000");

            auto const res = smf::mbus::radio::decode(h.get_server_id(), t.get_access_no(), aes, payload);

            // C-Field: 0x44 (SND_NR)
            // CI-Field: 0x7a (Parsbar)
            // AccessNumber: 42
            // id: 12345678
            // [2e][44][9315][78563412][33][03][7a] 2a0020055923c95aaa26d1b2e7493b013ec4a6f6d3529b520edff0ea6defc99d6d69ebf3/3ec8a
            BOOST_REQUIRE_EQUAL(res.size(), h.effective_payload_size());
            BOOST_REQUIRE_EQUAL(res.at(0), 0x2a);
            // BOOST_REQUIRE_EQUAL(res.at(1), 0x2f);

            //  Volume	28504.27  m3
            //  Time & Date	01.06.2008 01:50:00
            //  Status: 0
