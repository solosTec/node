...
int main(int argc, char **argv) 
{
	...
	generic.add_options()
		...
		("config,C", boost::program_options::value<std::string>(&config_file)->default_value(node::get_cfg_name("stat")), "specify the configuration file")
	...
	//	STAT task options
	//
	boost::program_options::options_description node_options("stat");
	...
	if (vm.count("version")) 
	{  
		return node::print_version_info(std::cout, "stat");
	}
	...
	try
	{
		...
		//
		node::controller ctrl(config_index, pool_size, json_path, "task:stat");
