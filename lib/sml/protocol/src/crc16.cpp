...
namespace node
{
	namespace sml	
	{
		...
		static std::uint16_t fcstab[256] = { 0x0000, 0x1189, 0x2312, 0x329b, 0x4624, 0x57ad, 0x6536, 0x74bf, 0x8c48,
		...
		std::uint16_t sml_crc16_calculate(cyng::buffer_t const& b)
		{
			BOOST_ASSERT_MSG(b.size() > 4, "insufficient buffer size");
			return (b.size() > 4)
				? sml_crc16_calculate((const unsigned char*)b.data(), b.size() - 4)
				: 0
				;
		}
	}
