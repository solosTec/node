...
namespace node
{
	namespace sml
	{
		namespace reader
		{
			...
			//cyng::vector_t read_msg(cyng::tuple_t::const_iterator, cyng::tuple_t::const_iterator);
			...
			cyng::vector_t read_msg(cyng::tuple_t::const_iterator pos, cyng::tuple_t::const_iterator end)
			{
				...
				ro.set_trx(cyng::value_cast(*pos++, buffer));
		...
		void read_sml_list(readout& ro, obis code
			...
			, cyng::tuple_t::const_iterator end)
		{
			...
			for (auto const child : tpl)
			{
				...
				cyng::tuple_t tree;
				tree = cyng::value_cast(child, tree);
		...
		cyng::param_t collect_devices(obis code, cyng::tuple_t const& tpl)
		{
			...
			for (auto const& outer_list : tpl) {
				...
				cyng::tuple_t devices;
				devices = cyng::value_cast(outer_list, devices);
				...
				if (devices.size() == 3) {
					...
					cyng::tuple_t inner_list;
					inner_list = cyng::value_cast(*pos, inner_list);
					...
					for (auto const& device : inner_list) {
						...
						values = cyng::value_cast(param.second, values);
						...
						if (pos_srv != values.end()) {
							...
							cyng::buffer_t meter;
							meter = cyng::value_cast(pos_srv->second, meter);
		...
		cyng::param_t collect_iec_devices(cyng::tuple_t const& tpl)
		{
			...
			for (auto const& outer_list : tpl) {
				...
				if (devices.size() == 3) {
					...
					cyng::tuple_t device;
					device = cyng::value_cast(*pos, device);
