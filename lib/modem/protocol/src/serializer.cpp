...
namespace node
{
	namespace modem
	{
		serializer::serializer(boost::asio::ip::tcp::socket& s
			...
			, ostream_(&buffer_)
		{
			vm.run(cyng::register_function("stream.flush", 0, [this, &s](cyng::context& ctx) {
			...
			vm.run(cyng::register_function("stream.serialize", 0, [this](cyng::context& ctx) {
			...
			//	after reconnect
			//
			vm.run(cyng::register_function("ipt.reset.serializer", 1, std::bind(&serializer::reset, this, std::placeholders::_1)));

			//
			//	transfer data
			//
			vm.run(cyng::register_function("modem.transfer.data", 1, std::bind(&serializer::transfer_data, this, std::placeholders::_1)));

			//
			//	hayes/AT specific commands
			//
			vm.run(cyng::register_function("print.ok", 0, std::bind(&serializer::print_msg, this, std::placeholders::_1, "OK")));
			vm.run(cyng::register_function("print.error", 0, std::bind(&serializer::print_msg, this, std::placeholders::_1, "ERROR")));
			vm.run(cyng::register_function("print.busy", 0, std::bind(&serializer::print_msg, this, std::placeholders::_1, "BUSY")));
			vm.run(cyng::register_function("print.no-dialtone", 0, std::bind(&serializer::print_msg, this, std::placeholders::_1, "NO DIALTONE")));
			...
			vm.run(cyng::register_function("res.open.push.channel", 8, std::bind(&serializer::res_open_push_channel, this, std::placeholders::_1)));
			...
			vm.run(cyng::register_function("res.open.connection", 2, std::bind(&serializer::res_open_connection, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.close.connection", 0, std::bind(&serializer::req_close_connection, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.close.connection", 2, std::bind(&serializer::res_close_connection, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.protocol.version", 0, std::bind(&serializer::req_protocol_version, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.protocol.version", 2, std::bind(&serializer::res_protocol_version, this, std::placeholders::_1)));
			...
			vm.run(cyng::register_function("req.device.auth", 0, std::bind(&serializer::req_device_auth, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.device.auth", 5, std::bind(&serializer::res_device_auth, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.device.time", 0, std::bind(&serializer::req_device_time, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.device.time", 1, std::bind(&serializer::res_device_time, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.push.target.namelist", 1, std::bind(&serializer::req_push_target_namelist, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.push.target.namelist", 1, std::bind(&serializer::res_push_target_namelist, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.push.target.echo", 1, std::bind(&serializer::req_push_target_echo, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.push.target.echo", 1, std::bind(&serializer::res_push_target_echo, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.traceroute", 4, std::bind(&serializer::req_traceroute, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.traceroute", 1, std::bind(&serializer::res_traceroute, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.maintenance", 1, std::bind(&serializer::req_maintenance, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.maintenance", 1, std::bind(&serializer::res_maintenance, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.logout", 1, std::bind(&serializer::req_logout, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.logout", 1, std::bind(&serializer::res_logout, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.register.push.target", 3, std::bind(&serializer::req_register_push_target, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.register.push.target", 3, std::bind(&serializer::res_register_push_target, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.deregister.push.target", 1, std::bind(&serializer::req_deregister_push_target, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.deregister.push.target", 3, std::bind(&serializer::res_deregister_push_target, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.watchdog", 1, std::bind(&serializer::req_watchdog, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.watchdog", 1, std::bind(&serializer::res_watchdog, this, std::placeholders::_1)));

			vm.run(cyng::register_function("req.multi.ctrl.public.login", 1, std::bind(&serializer::req_multi_ctrl_public_login, this, std::placeholders::_1)));
			vm.run(cyng::register_function("res.multi.ctrl.public.login", 1, std::bind(&serializer::req_multi_ctrl_public_login, this, std::placeholders::_1)));
			...
			vm.run(cyng::register_function("res.unknown.command", 1, std::bind(&serializer::res_unknown_command, this, std::placeholders::_1)));
