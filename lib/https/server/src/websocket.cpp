...
namespace node
{
	namespace https
	{
		...
		: websocket_session<plain_websocket>(logger
				, cb
		...
		void plain_websocket::do_timeout()
		{
		...
		void plain_websocket::on_close(boost::system::error_code ec)
		{
		...
		ssl_websocket::ssl_websocket(cyng::logging::log_ptr logger
			...
			, std::vector<std::string> const& sub_protocols)
		: websocket_session<ssl_websocket>(logger
				, cb
				, stream.get_executor().context(), sub_protocols)
			, ws_(std::move(stream))
		...
		// Called by the base class
		boost::beast::websocket::stream<ssl_stream<boost::asio::ip::tcp::socket>>& ssl_websocket::ws()
		{
			return ws_;
		}

		void ssl_websocket::do_eof()
		{
			eof_ = true;
		...
		void ssl_websocket::on_shutdown(boost::system::error_code ec)
		{
