...
#include "../../../nodes/print_version_info.h"
...
int main(int argc, char** argv)
{
	...
	generic.add_options()
		...
		("config,C", boost::program_options::value<std::string>(&config_file)->default_value(node::get_cfg_name("master")), "specify the configuration file")
		...
		//("interactive,I", boost::program_options::bool_switch()->default_value(true), "display prompt")
	...
	//	master node options
	//
	boost::program_options::options_description node_options("smf");
	...
	try {
		...
		if (vm.count("version"))
		{
			return node::print_version_info(std::cout, "smf");
		}
		...
		{
			//	run as service 
			::OutputDebugString("start SMF tool");
